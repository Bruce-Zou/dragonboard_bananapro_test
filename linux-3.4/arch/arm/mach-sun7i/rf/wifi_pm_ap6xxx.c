...
static int ap6xxx_gpio_ctrl(char* name, int level)
{
	...
	char * gpio_name[2] = {"ap6xxx_wl_regon", "ap6xxx_bt_regon", "ap6xxx_wl_vdd_en"};
...
void ap6xxx_power(int mode, int *updown)
{
    if (mode) {
        if (*updown) {
			ap6xxx_gpio_ctrl("ap6xxx_wl_vdd_en", 0);
			mdelay(100);
			ap6xxx_gpio_ctrl("ap6xxx_wl_regon", 1);
			mdelay(100);
        } else {
			ap6xxx_gpio_ctrl("ap6xxx_wl_vdd_en", 1);
			mdelay(100);
...
void ap6xxx_gpio_init(void)
{
	...
	if (SCIRPT_ITEM_VALUE_TYPE_PIO!=type) 
        ...
        type = script_get_item(wifi_para, "ap6xxx_wl_vdd_en", &val);
        if (SCIRPT_ITEM_VALUE_TYPE_PIO!=type) 
                ap6xxx_msg("get  ap6xxx_wl_vdd_en gpio failed\n");
        else
                ap6xxx_wl_vdd_en = val.gpio.gpio;
